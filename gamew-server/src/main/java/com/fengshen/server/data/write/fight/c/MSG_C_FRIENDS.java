 ...
 */
@Service
public class MSG_C_FRIENDS extends BaseWrite<List<Vo_ADD_FRIEND_OPPONENT>> {
	@Override
	protected void writeO(final ByteBuf writeBuf, List<Vo_ADD_FRIEND_OPPONENT> obj) {
		...
		for (int i = 0; i < obj.size(); ++i) {
			Vo_ADD_FRIEND_OPPONENT object2 = obj.get(i);
			//effectIcons
			if(object2.upgrade_type == 3) {
				GameWriteTool.writeByte(writeBuf, 1);
				GameWriteTool.writeInt(writeBuf, 8043);
			}else if(object2.upgrade_type == 4){
				GameWriteTool.writeByte(writeBuf, 1);
				GameWriteTool.writeInt(writeBuf, 8045);
			}else {
